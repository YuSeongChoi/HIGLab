...
const translations = {
  // New batch
  "Pie 차트는 5개 이하 항목에 적합합니다. 많은 항목에는 Bar를 사용하세요": "Pie charts work best with 5 or fewer items. Use Bar for more",
  "데이터 포인트가 너무 많으면 차트가 복잡해집니다": "Too many data points make the chart cluttered",
  "Grid 없이 Bar/Line 차트를 표시하면 값 비교가 어렵습니다": "Bar/Line charts without grid make value comparison difficult",
  "5개 이상의 데이터에는 범례(Legend)를 표시하세요": "Show legend for 5+ data points",
  "5개 이상의 탭은 라벨이 좁아져 읽기 어렵습니다": "5+ tabs make labels narrow and hard to read",
  "배지 숫자가 99를 초과하면 '99+'로 표시하는 것이 좋습니다": "Badge numbers above 99 should display as '99+'",
  "5개 이상의 세그먼트는 라벨이 잘립니다. 다른 UI를 고려하세요": "5+ segments cause label truncation. Consider alternative UI",
  "2개 아이콘만 있으면 Toggle이 더 적합할 수 있습니다": "With only 2 icons, a Toggle might be more appropriate",
  "빨간색 토글은 '위험' 의미로 오해될 수 있습니다. 초록색을 권장합니다": "Red toggle tint can be confused with 'danger'. Green is recommended",
  "빨간색 슬라이더는 경고 의미로 오해될 수 있습니다": "Red slider tint can be misinterpreted as a warning",
  "진행률 0%는 시작 전인지 오류인지 구분이 안 됩니다. 로딩 인디케이터를 사용하세요": "0% progress is ambiguous — use a loading indicator for indeterminate state",
  "스크롤 인디케이터 없이는 더 많은 콘텐츠가 있는지 알 수 없습니다": "Without scroll indicators, users can't tell there's more content",
  "Vertical 페이징은 일반적이지 않습니다. Horizontal 페이징이 더 자연스럽습니다": "Vertical paging is uncommon. Horizontal paging feels more natural",
  "3단계 이상 중첩은 사용자가 현재 위치를 파악하기 어렵습니다": "3+ nesting levels make it hard for users to track their position",
  "너무 많은 노드가 펼쳐져 있으면 전체 구조 파악이 어렵습니다": "Too many expanded nodes make it hard to see the overall structure",
  "5개 이상의 컬럼은 모바일에서 읽기 어렵습니다. 핵심 데이터만 표시하세요": "5+ columns are hard to read on mobile. Show only essential data",
  // Previous batch (for completeness)
...
for (const file of enFiles) {
  ...
  if (!html.includes('higRules') && !html.includes('hig-violations')) continue;
...
console.log(`Done: ${count} EN files with translated violations`);
