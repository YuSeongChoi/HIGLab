#!/bin/bash
# 모든 튜토리얼 DocC 빌드 스크립트
# 사용법: ./build-docc-all.sh <lang> <output_base>
# 예시: ./build-docc-all.sh en deploy/tutorials/en

set -e

LANG=${1:-en}
OUTPUT_BASE=${2:-deploy/tutorials/en}
SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
BASE_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
TUTORIALS_DIR="$BASE_DIR/tutorials"

# 튜토리얼 목록 (scheme 이름 매핑)
declare -A TUTORIALS=(
  ["widgets"]="HIGWidgets"
...
for name in "${!TUTORIALS[@]}"; do
  scheme="${TUTORIALS[$name]}"
  
  echo "🔨 Building $name ($scheme)..."
  
  # 언어 전환
  "$SCRIPT_DIR/docc-set-lang.sh" "$LANG" "$name"
  
  cd "$TUTORIALS_DIR/$name"
  ...
  if [[ -d "$archive_path" ]]; then
    $(xcrun --find docc) process-archive \
      ...
      --hosting-base-path "HIGLab/tutorials/$LANG/$name" 2>/dev/null
