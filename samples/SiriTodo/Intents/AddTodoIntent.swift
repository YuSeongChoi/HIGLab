    ...
    static var parameterSummary: some ParameterSummary {
        When(\.$dueDatePreset, .equalTo, .none) {
            When(\.$priority, .equalTo, .normal) {
                ...
                Summary("'\(\.$title)' \(\.$priority) 우선순위로 추가")
            }
        } otherwise: {
            When(\.$priority, .equalTo, .normal) {
                Summary("'\(\.$title)' \(\.$dueDatePreset)까지 추가")
            } otherwise: {
                Summary("'\(\.$title)' \(\.$priority) 우선순위로 \(\.$dueDatePreset)까지 추가")
            }
        }
    }
}
...
struct QuickAddTodoIntent: AppIntent {
    ...
    static var title: LocalizedStringResource = "빠른 할일 추가"
...
struct AddTodayTodoIntent: AppIntent {
    ...
    static var title: LocalizedStringResource = "오늘 할일 추가"
...
struct AddUrgentTodoIntent: AppIntent {
    ...
    static var title: LocalizedStringResource = "긴급 할일 추가"
